...
class ContentController extends Controller
{
    ...
    public function create(): View { return view('admin.form', ['item' => new ContentItem]); }
    ...
    public function edit(ContentItem $contentItem): View { return view('admin.form', ['item' => $contentItem]); }
    ...
    {
        if ($contentItem->file_path) Storage::disk('public')->delete($contentItem->file_path);
    ...
    {
        $base = Str::slug($title) ?: 'item'; $slug = $base; $i = 2;
        while (ContentItem::where('slug', $slug)->when($ignoreId, fn ($q) => $q->whereKeyNot($ignoreId))->exists()) $slug = $base.'-'.$i++;
        return $slug;
    }
}
